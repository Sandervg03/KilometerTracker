...
export class Trip {
    ...
    async save(token: string): Promise<InsertTripMutation> {
        ...
        const lastTrip: Trip = await Trip.getLastTrip(this.group, token);

        if (lastTrip.currentMileage > this.currentMileage) {
    ...
    static async getLastFilledTankTrip(groupId: string, token: string): Promise<Trip> {
    ...
    static async getLastTrip(groupId: string, token: string): Promise<Trip> {
    ...
    static async getByGroupId(groupId: string, token: string): Promise<Trip[]> {
        const lastFilledTank: Trip = await this.getLastFilledTankTrip(groupId, token);

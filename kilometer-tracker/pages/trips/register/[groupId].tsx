...
export default function RegisterTrip() {
  ...
  const [formData, setFormData] = useState({
    currentMileage: 0,
    email: "", // Changed from null to empty string
    filledTank: false,
    groupId: groupId,
  });
    ...
    async function getData() {
      try {
        const response = await fetch(
          `/api/group/user/getGroup?groupId=${groupId}`,
          {
            method: "GET",
            headers: {
          ...
          <>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <input
                ...
                required
              />
              <div className={styles.formItem}>Who made this trip?</div>
              <select
                name="email"
